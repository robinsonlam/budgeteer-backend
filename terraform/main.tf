# Minimal Terraform for ECS deployment
...
terraform {
  ...
  backend "s3" {
    bucket  = "budgeteer-terraform-state-robinsonlam"
...
resource "aws_ecs_task_definition" "app" {
  ...
  container_definitions = jsonencode([{
    ...
    environment = [
      ...
      { name = "CORS_ORIGINS", value = var.cors_origins != "" ? var.cors_origins : "http://localhost:5173" },
...
resource "aws_ecs_service" "main" {
  ...
  load_balancer {
    target_group_arn = aws_lb_target_group.app.arn
    container_name   = var.app_image_name
